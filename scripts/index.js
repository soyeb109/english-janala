...
document.getElementById("getStartedBtn").addEventListener("click", function () {
 const name = document.getElementById("name").value;
  ...
  if (password === "123456") {
    ...
    "Incorrect password contact your admin to get your password!"
  }
});
...
const showLevels = (lessons) => {
  ...
  lessons.forEach((element) => {
    ...
    lessonDiv.innerHTML = `
    <button   id="lesson-btn-${element.level_no}"   onclick = "loadWords('${element.level_no}', this)"  class="text-[#422AD5] border border-bg-[#422AD5] px-2 py-2 hover:bg-[#422AD5] hover:text-[#C6BDBD] rounded cursor-pointer "> <i class="fa-solid fa-book-open "></i> ${element.lessonName}</button>
    `
    // append
...
const loadWords = async (levels) => {
  ...
  try {
    const response = await fetch(`https://openapi.programming-hero.com/api/level/${levels}`);
...
const displayErrorPage = () => {
  ...
  wordsContainer.innerHTML = `
    ...
    <img src="/assets/alert-error.png">
      <h2 class="text-2xl font-bold text-gray-600 noto-serif py-4">এই Lesson এ এখনো কোন Vocabulary যক্ত করা হয়নি।</h2>
...
const displayWordDetails = (words) => {
  ...
  const synonymText = words.synonyms ? words.synonyms :  "";


  detailsContainer.innerHTML = `
  <div class="flex gap-3">
  <h1 class="text-2xl font-bold">${words.word}</h1>
  (<i class="fa-solid fa-microphone"></i>): <h1 class="noto-serif">${words.pronunciation}</h1>
  </div>
